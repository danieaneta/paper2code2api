...
Produces (into ./assets):
  * examples_grid.png      - a grid of input digits with predicted label + confidence,
                             green border = correct, red = wrong.
  * prediction_detail.png  - a single input digit next to its full output probability bar chart.
...
N_GRID = 10  # number of digits in the grid
...
def main() -> None:
    ...
    fig.suptitle("LeNet-5 — inputs (MNIST) and predictions", fontsize=13, fontweight="bold")
    ...
    for i, ax in enumerate(axes.flat):
        ...
        ax.set_title(f"pred {pred}  ({conf*100:.1f}%)\ntrue {true_label}",
